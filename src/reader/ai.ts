...
export const aiRules: SiteRule[] = [
  ...
  },
  {
    ...
    pick: $ => {
      const $body = $('article').first().clone()
      $body.find('[class*="related"], [class*="newsletter"], [class*="social"], footer').remove()
      // Strip AI-generated summary sections and trailing metadata
      $body.find('*').each((_, el) => {
        const t = $(el).text().trim()
        if (/^(General summary|Bullet points|Summaries were generated by Google AI|POSTED IN:)$/i.test(t)) {
          $(el).nextAll().remove()
          $(el).remove()
        }
      })
