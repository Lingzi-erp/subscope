...
Usage: python cffi_fetch.py <url> [impersonate]
Outputs HTML to stdout. Exits 1 on failure."""
...
# Try requested impersonation, fallback chain if 403
targets = [imp] if imp != 'auto' else ['safari17_0', 'safari15_3', 'chrome131']

for t in targets:
    try:
        r = requests.get(url, impersonate=t, timeout=15)
