...
const INSECURE_HOSTS = [
  // US/EU
  'bls.gov', 'bea.gov', 'sec.gov', 'treasury.gov', 'imf.org', 'ftc.gov', 'cftc.gov',
...
export const fetchWithCffi = async (url: string, impersonate = 'safari17_0', headers?: Record<string, string>, cookies?: Record<string, string>): Promise<string> => {
  const script = join(import.meta.dir, 'cffi_fetch.py')
  const args = ['python', script, url, impersonate,
